...
// note. it randomly fails due to linux killing the process. When it happens, just delete instance and run it again :D (or -replace aws_instance.database)
resource "aws_instance" "database" {
    ...
    user_data = <<-EOL
        ...
        S3_FILE="s3://\$BUCKET_NAME/backup-\$TIMESTAMP.gz"
...
resource "aws_s3_bucket_lifecycle_configuration" "dbbackup_lifecycle" {
  bucket = aws_s3_bucket.dbbackup.id
  rule {
    id = "To Glacier"
    status = "Enabled"
